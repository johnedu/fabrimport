...
appModule.config([
    ...
    function ($stateProvider, $urlRouterProvider, $qProvider) {
        ...
        if (abp.auth.hasPermission('Pages.Administration.SeedConfiguration')) {
            $stateProvider.state('seedConfiguration', {
                url: '/seed',
                component: 'seedConfiguration'
            });
        }
        ...
        if (abp.auth.hasPermission('Pages.Tenant.General.Zoning')) {
            $stateProvider.state('general.zoning', {
                url: '/zoning',
                component: 'generalZoning'
            });
        }

        if (abp.auth.hasPermission('Pages.Tenant.General.Products')) {
        ...
        if (abp.auth.hasPermission('Pages.Tenant.General.Customers')) {
            $stateProvider.state('general.customers', {
                url: '/customers',
                component: 'generalCustomers'
            });
        }

        if (abp.auth.hasPermission('Pages.Tenant.General.Employees')) {
            $stateProvider.state('general.employees', {
                url: '/employees',
                component: 'generalEmployees'
            });
        }

        if (abp.auth.hasPermission('Pages.Tenant.General.Providers')) {
            $stateProvider.state('general.providers', {
                url: '/providers',
                component: 'generalProviders'
            });
        }
        ...
        if (abp.auth.hasPermission('Pages.Tenant.Sales.Orders')) {
        ...
        if (abp.auth.hasPermission('Pages.Tenant.Sales.Invoices')) {
            $stateProvider.state('sales.invoices', {
                url: '/invoices',
                component: 'salesInvoices'
            });
        }

        if (abp.auth.hasPermission('Pages.Tenant.Sales.Inventory')) {
            $stateProvider.state('sales.inventory', {
                url: '/inventory',
...
appModule.run(["$rootScope", "settings", "$state", 'i18nService', '$uibModalStack',
    function ($rootScope, settings, $state, i18nService, $uibModalStack) {
